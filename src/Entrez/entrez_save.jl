...
function save_efetch_sqlite{T}(efetch_dict, db_config::Dict{Symbol, T}, verbose=false)
    db = init_database_sqlite(db_config)
    return save_efetch(efetch_dict, db, verbose)
end
...
function save_efetch_mysql{T}(efetch_dict, db_config::Dict{Symbol, T}, verbose=false)
    db = init_database_mysql(db_config)
    return save_efetch(efetch_dict, db, verbose)
end
...
Save the results (dictionary) of an entrez fetch to the input database.
"""
function save_efetch(efetch_dict, db, verbose=false)
